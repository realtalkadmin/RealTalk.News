...
function paperback_setup() {
  ...
  // Large post image
  add_image_size( 'paperback-full-width', 1200 );

  // Grid thumbnail
  add_image_size( 'paperback-grid-thumb', 375, 250, true );
...
function paperback_scripts() {
  ...
  */
   ...
   */
  if( 'enabled' == get_theme_mod( 'paperback_fixed_bar', 'enabled' ) ) { $load_fixed = 'true'; } else { $load_fixed = 'false'; }
 ...
 */
require get_template_directory() . '/inc/main-template.php';
...
function paperback_exclude_featured_posts($query) {
  ...
  if ( 1 === get_theme_mod( 'paperback_hero_header_exclude', 'disabled' ) && $query->is_home() && $query->is_main_query() ) {
